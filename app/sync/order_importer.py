...
class OrderImporter:
    ...
    # now shared via PieceWriter -- see app/db/piece_writer.py) -----------
    def _gen_value(self, generator):
        return self._pw.gen_value(generator)

    def _advance_generator(self, generator, target):
        self._pw.advance_generator(generator, target)

    def _next_base(self, generator, table, col):
        return self._pw.next_base(generator, table, col)
    ...
    def _type_piece_coefficients(self, code_type_piece):
        """See PieceWriter.type_piece_coefficients -- kept as a thin
        wrapper here since existing tests call this via the importer."""
        return self._pw.type_piece_coefficients(code_type_piece)
